...
using Goosetrap.Models.APIs.Roblox;
using Goosetrap.RobloxInterfaces;
using Goosetrap.UI.Elements.Bootstrapper.Base;
...
namespace Goosetrap
{
    public class Bootstrapper
    {
        ...
        public string MutexName { get; set; } = "Goosetrap-Bootstrapper";
        ...
        public Bootstrapper(LaunchMode launchMode)
        {
            ...
            // exceptions don't get thrown if we define events without actually binding to the failure events. probably a bug. ВЇ\_(гѓ„)_/ВЇ
        ...
        public async Task Run()
        {
            ...
            if (!_noConnection)
            {
                ...
                {
                    bool backgroundUpdaterMutexOpen = Utilities.DoesMutexExist("Goosetrap-BackgroundUpdater");
        ...
        private static void LaunchMultiInstanceWatcher()
        {
            ...
            using EventWaitHandle initEventHandle = new EventWaitHandle(false, EventResetMode.AutoReset, "Goosetrap-MultiInstanceWatcherInitialisationFinished");
        ...
        private async void StartRoblox()
        {
            ...
            string ResolvedName = null!;
                ...
                // the code you're gonna read ahead is horrible. sorry for the hack, but it works ВЇ\_(гѓ„)_/ВЇ
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one Goosetrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingGoosetrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "Goosetrap.exe");
        ...
        private static void StartBackgroundUpdater()
        {
            ...
            if (Utilities.DoesMutexExist("Goosetrap-BackgroundUpdater"))
            {
