...
namespace Goosetrap
{
    internal class Installer
    {
        ...
        public string GoosetrapInstallDirectory = Path.Combine(Paths.LocalAppData, "Goosetrap"); // default directory for Goosetrap
        ...
        public bool ImportSettings = Directory.Exists(Path.Combine(Paths.LocalAppData, "Goosetrap")); // if Goosetrap isnt detected this will be set to false
        ...
        public void DoInstall()
        {
            ...
            // only register player, for the scenario where the user installs Goosetrap, closes it,
            ...
            if (ImportSettings)
            {
                ...
                // if something doenst exist Goosetrap will recreate the file/directory
                try
                {
                    ImportSettingsFromGoosetrap();
                } catch (Exception ex)
                {
        ...
        public static void HandleUpgrade()
        {
            ...
            if (existingVer is not null)
            {
                ...
                if (Utilities.CompareVersions(existingVer, "2.8.0") == VersionComparison.LessThan)
                {
                    ...
                    string oldStartPath = Path.Combine(Paths.WindowsStartMenu, "Goosetrap");
                    ...
                    Registry.CurrentUser.DeleteSubKeyTree("Software\\Goosetrap", false);
...
#pragma warning disable CS0162 // Unreachable code detected
                if (OpenReleaseNotes)
                    Utilities.ShellExecute($"https://github.com/{App.ProjectRepository}/wiki/Release-notes-for-Goosetrap-v{currentVer}");
#pragma warning restore CS0162 // Unreachable code detected
        ...
        public void ImportSettingsFromGoosetrap()
        {
            const string LOG_IDENT = "Installer::ImportSettings";

            if (!Directory.Exists(GoosetrapInstallDirectory))
            {
                Frontend.ShowMessageBox(Strings.Installer_InstallationNotFound, MessageBoxImage.Exclamation);
                return;
            } // Goosetrap default directory is not present

            foreach (string FileName in FilesForImporting)
            {
                string Source = Path.Combine(GoosetrapInstallDirectory, FileName);
                ...
                if (IsDirectory)
                {
                    // delete existing file from Goosetrap folder
                    ...
                    // we could use Directory.Move but that deletes the directory from Goosetrap folder
