namespace Goosetrap
{
    internal static class MultiInstanceWatcher
    {
        private static int GetOpenProcessesCount()
        {
            ...
            try
            {
                // prevent any possible race conditions by checking for Goosetrap processes too
                int count = Process.GetProcesses().Count(x => x.ProcessName is "RobloxPlayerBeta" or "Goosetrap");
